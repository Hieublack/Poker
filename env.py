...
def reset_round(old_env_state):
    ...
    #người chơi action kế tiếp là người thứ 3 từ button dealer
    env_state[ENV_ID_ACTION] = (env_state[ENV_BUTTON_PLAYER] + 3) % NUMBER_PLAYER
    #duyệt người chơi action cho đến khi gặp người chơi còn chip
...
def holdem(board, hands):
    ...
    all_best_hand_player = []
    for result in scores:
        all_best_hand_player.append(result[2])
    scores = sorted(scores, reverse= True)
    # for result in scores:
    #     print(result[0], result[1], ALL_CARD_STR[np.array(result[2])], result[2], result[3])
...
def showdown(env_state):
    ...
    if np.count_nonzero(status) == 1:
        ...
        for i in range(1, 9):
            ...
            #     # env_state[ENV_ALL_FIRST_CARD_SHOWDOWN + id] = -1
                # env_state[ENV_ALL_FIRST_CARD_SHOWDOWN + id] = -1
    #print('receive', player_chip_receive)
    #print('resu;t', env_state[ENV_ALL_PLAYER_CHIP : ENV_ALL_PLAYER_CHIP_GIVE])
    return env_state
...
def checkEnded(env_state):
    ...
    if np.count_nonzero(all_player_chip) == 1 or number_game == 200:
        #print(all_player_chip)
        return True
    return False
...
def getReward(agent_state):
    ...
    if np.count_nonzero(all_player_status) == 1 or agent_state[P_NUMBER_GAME_PLAY] == 200:
